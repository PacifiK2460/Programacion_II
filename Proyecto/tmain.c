...
DWORD WINAPI InitOutput()
{
    while (1)
    {
        // Update new number of layers / threads to every layer
        int layers = 0;
        layers = outputSubscriber.subscribers.size;

        DWORD dwThreadIdArray[layers];
        HANDLE hThreadArray[layers];

        for (int i = 0; i < layers; i++)
        {
            POutputSubsciption outputSubscription = getNode(&outputSubscriber.subscribers, i);

            hThreadArray[i] = CreateThread(
                NULL,                          // default security attributes
                0,                             // use default stack size
                outputSubscription.Render,     // thread function name
                outputSubscription.RenderArgs, // argument to thread function
                0,                             // use default creation flags
